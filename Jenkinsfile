			    ...
			    script {
                  container('docker') {
					 ...
					 withCredentials([usernamePassword(credentialsId: 'DOCKER_CRED', 
						...
						sh 'docker images'
						sh "docker tag $username/${imageName}:${build_version} ${dockerRegistry}/${imageName}:${build_version}"
						sh "docker tag $username/${imageName}:${build_version} ${dockerRegistry}/${imageName}:latest"
